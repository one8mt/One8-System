...
import { UserCircle, CheckCircle, ShoppingCart } from 'lucide-react';
...
import { Stars, InvoiceModal, initialPmsData } from './financeShared';
import type { DataEntry, AccountingEntry } from './financeShared';
...
interface FinancePMSProps {
    submittedIds: Set<string>;
    onSubmit: (entry: DataEntry, source: 'PMS' | 'CRM') => void;
}

export function FinancePMS({ submittedIds, onSubmit }: FinancePMSProps) {
    ...
    return (
        <>
            <Card>
                <CardHeader>
                    <CardTitle>PMS Invoices</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            {selected && (
                <InvoiceModal
                    ...
                    onSubmit={(updated) => { onSubmit(updated, 'PMS'); setSelected(null); }}
                />
            )}
        </>
    );
